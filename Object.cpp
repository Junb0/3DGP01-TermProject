...
#define _WITH_DEBUG_RUNTIME_FRAME_HIERARCHY
...
CTankObject::CTankObject()
{
	...
	m_fAccelSpeedXZ = 1.0f;
	m_fSpeedRotateY = 90.0f;

	m_nAccelKey = VK_UP;
	m_nDeccelKey = VK_DOWN;
	m_nLeftRotateKey = VK_LEFT;
	m_nRightRotateKey = VK_RIGHT;
	m_nFireKey = VK_RETURN;
...
void CTankObject::BuildBullets(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,ID3D12RootSignature* m_pd3dGraphicsRootSignature, int nBullets)
{
	...
	for (int i = 0; i < m_nBullets; i++)
	{
		...
		pNewBulletModel = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, "Model/Shell.bin");

		pBulletObject = new CBulletObject();
		pBulletObject->SetChild(pNewBulletModel, true);
		pBulletObject->OnInitialize();
...
void CTankObject::Release()
{
	if (m_nBullets)
	{
		for (int i = 0; i < m_nBullets; i++)
			if (m_ppBullets[i])
			{
				m_ppBullets[i]->Release();
				TCHAR pstrDebug[256] = { 0 };
				_stprintf_s(pstrDebug, 256, _T("(Release Bullet: %p)\n"), m_ppBullets[i]);
				OutputDebugString(pstrDebug);
			}
		delete[] m_ppBullets;
		TCHAR pstrDebug[256] = { 0 };
		//_stprintf_s(pstrDebug, 256, _T("(Child Frame: %p) (Parent Frame: %p)\n"), pChild, pGameObject);
		_stprintf_s(pstrDebug, 256, _T("Delete Bullets\n"));
		OutputDebugString(pstrDebug);
	}
	CGameObject::Release();
}
...
{
	m_pTankTurret = FindFrame("TankTurret");
	if (m_pTankTurret) m_xmf4x4OriginalTurretTransform = m_pTankTurret->m_xmf4x4Transform;
}
...
void CTankObject::OnPrepareRender()
{
	...
	SetScale(m_fScaleX, m_fScaleY, m_fScaleZ);
...
bool CTankObject::DamagedByBullet(CBulletObject* pBulletObject)
{
	...
	/*TCHAR pstrDebug[256] = { 0 };
	_stprintf_s(pstrDebug, 256, _T("(Collision Bullet LOOK : %f %f %f)\n"),
		pBulletObject->GetLook().x, pBulletObject->GetLook().y, pBulletObject->GetLook().z
	);
	OutputDebugString(pstrDebug);*/
}
...
{
	if (m_bIsActive) CRotatingObject::Render(pd3dCommandList, pCamera);
}
